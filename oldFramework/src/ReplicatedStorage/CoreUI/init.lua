--[[
	...
	This system manages UI modules that are paired with UI instances in a ScreenGui.
	UI elements are created declaratively in Roblox Studio, and modules control their behavior.
	
	Usage:
		1. Create a ScreenGui with UI elements (named frames, etc.)
		2. Create a module in CoreUI/Modules/ with a name matching a UI element
		3. The module must export a `start(export, ui)` function
		4. Initialize with: CoreUI.new(screenGui):init()
]]
...
local ConnectionManager = require(ReplicatedStorage.Utils.ConnectionManager)
local TweenLibrary = require(ReplicatedStorage.Utils.TweenLibrary)
