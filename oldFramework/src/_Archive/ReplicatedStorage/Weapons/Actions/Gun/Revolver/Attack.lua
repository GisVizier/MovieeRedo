--[[
	...
	Handles firing the Revolver weapon with client-side hitscan.
]]
...
function Attack.Execute(weaponInstance)
	...
	if currentTime - state.LastFireTime < cooldown then
		print("[Revolver] Cooldown not ready")
		return
	end

	print("[Revolver] Firing for player:", player.Name)
	...
	if not camera then
		warn("[Revolver] No camera found")
		return
	end

	local character = player.Character
	if not character then
		warn("[Revolver] No character found")
		return
	end
	...
	if not humanoidHead then
		warn("[Revolver] No humanoid head found on character")
		return
	end
	...
	print("[Revolver] Raycast Origin:", origin)
	print("[Revolver] Raycast Direction:", direction)
	print("[Revolver] Raycast Max Range:", maxRange)
	if isShotgun then
		print("[Revolver] SHOTGUN MODE - Pellets:", pelletCount, "Spread:", pelletSpread)
	end
		...
		if raycastResult then
			...
			if isHitboxHit then
				...
				if isHeadshotHit then
					...
					print("[Revolver] DEBUG - Pellet", pelletIndex, "HEADSHOT!")
				else
					beam.Color = ColorSequence.new(Color3.new(0, 1, 0)) -- Green = body shot
					print("[Revolver] DEBUG - Pellet", pelletIndex, "HIT PLAYER BODY!")
				end
			else
				beam.Color = ColorSequence.new(Color3.new(0, 0, 0)) -- Black = wall/object hit
				print("[Revolver] DEBUG - Pellet", pelletIndex, "Hit wall/object (not a player)")
			end

			print("[Revolver] DEBUG - Pellet", pelletIndex, "Hit Instance:", raycastResult.Instance:GetFullName())
			print("[Revolver] DEBUG - Pellet", pelletIndex, "Hit Position:", raycastResult.Position)
			print("[Revolver] DEBUG - Pellet", pelletIndex, "Distance:", raycastResult.Distance)
		else
			beamAttachment1.WorldPosition = origin + direction
			beam.Color = ColorSequence.new(Color3.new(0, 0, 0)) -- Black = complete miss
			print("[Revolver] DEBUG - Pellet", pelletIndex, "No hit detected")
		end
	...
	for pelletIndex, raycastResult in ipairs(raycastResults) do
		if raycastResult then
			...
			if isHitboxHit then
				...
				print("[Revolver] Pellet", pelletIndex, "HIT PLAYER!", {
				...
				if hitInfo.IsHeadshot then
					print("[Revolver] Pellet", pelletIndex, "HEADSHOT CONFIRMED!")
				end
			else
				print("[Revolver] Pellet", pelletIndex, "HIT WALL/OBJECT", {
	...
	if #hitData.Hits == 0 then
		print("[Revolver] All pellets missed")
	end
	...
	print("[Revolver] Ammo remaining:", state.CurrentAmmo, "/", config.Ammo.MagSize)
