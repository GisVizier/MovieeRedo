--[[
	Reload.lua (SniperRifle)

	Handles reloading the SniperRifle weapon.
]]
...
function Reload.Execute(weaponInstance)
	...
	if not BaseGun.CanReload(weaponInstance) then
		print("[SniperRifle] Cannot reload - conditions not met")
		return
	end

	print("[SniperRifle] Reloading for player:", player.Name)
	print("[SniperRifle] Current ammo:", state.CurrentAmmo, "Reserve:", state.ReserveAmmo)
	...
	print("[SniperRifle] Reload complete. Ammo:", state.CurrentAmmo, "/", config.Ammo.MagSize, "Reserve:", state.ReserveAmmo)
end
