--[[
	...
	Handles firing the Shotgun weapon with client-side hitscan.
	Supports shotgun mode with multiple pellets.
]]
...
function Attack.Execute(weaponInstance)
	...
	if currentTime - state.LastFireTime < cooldown then
		print("[Shotgun] Cooldown not ready")
		return
	end

	print("[Shotgun] Firing for player:", player.Name)
	...
	if not camera then
		warn("[Shotgun] No camera found")
		return
	end

	local character = player.Character
	if not character then
		warn("[Shotgun] No character found")
		return
	end
	...
	if not humanoidHead then
		warn("[Shotgun] No humanoid head found on character")
		return
	end
	...
	print("[Shotgun] Raycast Origin:", origin)
	print("[Shotgun] Raycast Direction:", direction)
	print("[Shotgun] Raycast Max Range:", maxRange)
	if isShotgun then
		print("[Shotgun] SHOTGUN MODE - Pellets:", pelletCount, "Spread:", pelletSpread)
	end
	...
	if isShotgun then
		...
		if singleResult then
			raycastResults = {{Result = singleResult, Direction = direction}}
		end
	end

	-- DEBUG: Visualize all raycast paths
	for pelletIndex, pelletData in ipairs(raycastResults) do
		local raycastResult = pelletData.Result
		local pelletDirection = pelletData.Direction
		...
		if raycastResult then
			...
			if isHitboxHit then
				...
				if isHeadshotHit then
					...
					print("[Shotgun] DEBUG - Pellet", pelletIndex, "HEADSHOT!")
				else
					beam.Color = ColorSequence.new(Color3.new(0, 1, 0)) -- Green = body shot
					print("[Shotgun] DEBUG - Pellet", pelletIndex, "HIT PLAYER BODY!")
				end
			else
				beam.Color = ColorSequence.new(Color3.new(0.5, 0.5, 0.5)) -- Gray = wall/object hit
				print("[Shotgun] DEBUG - Pellet", pelletIndex, "Hit wall/object (not a player)")
			end

			print("[Shotgun] DEBUG - Pellet", pelletIndex, "Hit Instance:", raycastResult.Instance:GetFullName())
			print("[Shotgun] DEBUG - Pellet", pelletIndex, "Hit Position:", raycastResult.Position)
			print("[Shotgun] DEBUG - Pellet", pelletIndex, "Distance:", raycastResult.Distance)
		else
			-- No hit - extend beam to max range in the pellet's actual direction
			beamAttachment1.WorldPosition = origin + pelletDirection
			beam.Color = ColorSequence.new(Color3.new(0.5, 0.5, 0.5)) -- Gray = complete miss (not hitting a person)
			print("[Shotgun] DEBUG - Pellet", pelletIndex, "No hit detected - beam extends to max range")
		end
	...
	for pelletIndex, pelletData in ipairs(raycastResults) do
		...
		if raycastResult then
			...
			if isHitboxHit then
				...
				local targetPlayer = HitscanSystem:GetPlayerFromHit(raycastResult.Instance)
				-- IMPORTANT: Send UserId, not Player object (Player objects don't serialize over RemoteEvents)
				hitInfo.TargetUserId = targetPlayer and targetPlayer.UserId or nil

				print("[Shotgun] Pellet", pelletIndex, "HIT PLAYER!", {
					Player = targetPlayer and targetPlayer.Name or "Unknown",
					UserId = hitInfo.TargetUserId,
				...
				if hitInfo.IsHeadshot then
					print("[Shotgun] Pellet", pelletIndex, "HEADSHOT CONFIRMED!")
				end
			else
				print("[Shotgun] Pellet", pelletIndex, "HIT WALL/OBJECT", {
	...
	print("[Shotgun] Ammo remaining:", state.CurrentAmmo, "/", config.Ammo.MagSize)
