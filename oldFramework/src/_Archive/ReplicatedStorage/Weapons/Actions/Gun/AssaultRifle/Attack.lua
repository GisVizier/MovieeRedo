--[[
	...
	Handles firing the AssaultRifle weapon with client-side hitscan.
]]
...
function Attack.Execute(weaponInstance)
	...
	if not BaseGun.CanFire(weaponInstance) then
		print("[AssaultRifle] Cannot fire - conditions not met")

		-- Play dry fire sound if out of ammo
		if state.CurrentAmmo <= 0 then
			-- TODO: Play dry fire sound
			print("[AssaultRifle] Out of ammo - dry fire")
		end
	...
	if currentTime - state.LastFireTime < cooldown then
		print("[AssaultRifle] Cooldown not ready")
		return
	end

	print("[AssaultRifle] Firing for player:", player.Name)
	...
	if not camera then
		warn("[AssaultRifle] No camera found")
		return
	end

	local character = player.Character
	if not character then
		warn("[AssaultRifle] No character found")
		return
	end
	...
	if not humanoidHead then
		warn("[AssaultRifle] No humanoid head found on character")
		return
	end
	...
	print("[AssaultRifle] Raycast Origin:", origin)
	print("[AssaultRifle] Raycast Direction:", direction)
	print("[AssaultRifle] Raycast Max Range:", maxRange)
	if isShotgun then
		print("[AssaultRifle] SHOTGUN MODE - Pellets:", pelletCount, "Spread:", pelletSpread)
	end
		...
		if raycastResult then
			...
			if isHitboxHit then
				...
				if isHeadshotHit then
					...
					print("[AssaultRifle] DEBUG - Pellet", pelletIndex, "HEADSHOT!")
				else
					beam.Color = ColorSequence.new(Color3.new(0, 1, 0)) -- Green = body shot
					print("[AssaultRifle] DEBUG - Pellet", pelletIndex, "HIT PLAYER BODY!")
				end
			else
				beam.Color = ColorSequence.new(Color3.new(0, 0, 0)) -- Black = wall/object hit
				print("[AssaultRifle] DEBUG - Pellet", pelletIndex, "Hit wall/object (not a player)")
			end

			print("[AssaultRifle] DEBUG - Pellet", pelletIndex, "Hit Instance:", raycastResult.Instance:GetFullName())
			print("[AssaultRifle] DEBUG - Pellet", pelletIndex, "Hit Position:", raycastResult.Position)
			print("[AssaultRifle] DEBUG - Pellet", pelletIndex, "Distance:", raycastResult.Distance)
		else
			beamAttachment1.WorldPosition = origin + direction
			beam.Color = ColorSequence.new(Color3.new(0, 0, 0)) -- Black = complete miss
			print("[AssaultRifle] DEBUG - Pellet", pelletIndex, "No hit detected")
		end
	...
	for pelletIndex, raycastResult in ipairs(raycastResults) do
		if raycastResult then
			...
			if isHitboxHit then
				...
				print("[AssaultRifle] Pellet", pelletIndex, "HIT PLAYER!", {
				...
				if hitInfo.IsHeadshot then
					print("[AssaultRifle] Pellet", pelletIndex, "HEADSHOT CONFIRMED!")
				end
			else
				print("[AssaultRifle] Pellet", pelletIndex, "HIT WALL/OBJECT", {
	...
	if #hitData.Hits == 0 then
		print("[AssaultRifle] All pellets missed")
	end
	...
	print("[AssaultRifle] Ammo remaining:", state.CurrentAmmo, "/", config.Ammo.MagSize)
	...
	if state.CurrentAmmo <= 0 and config.Ammo.AutoReload and state.ReserveAmmo > 0 then
		print("[AssaultRifle] Auto-reloading...")
