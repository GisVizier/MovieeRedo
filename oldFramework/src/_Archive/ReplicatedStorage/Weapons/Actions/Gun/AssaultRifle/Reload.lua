--[[
	...
	Handles reloading the AssaultRifle weapon.
]]
...
function Reload.Execute(weaponInstance)
	...
	if not BaseGun.CanReload(weaponInstance) then
		print("[AssaultRifle] Cannot reload - conditions not met")
		return
	end

	print("[AssaultRifle] Reloading for player:", player.Name)
	print("[AssaultRifle] Current ammo:", state.CurrentAmmo, "Reserve:", state.ReserveAmmo)
	...
	print("[AssaultRifle] Reload complete. Ammo:", state.CurrentAmmo, "/", config.Ammo.MagSize, "Reserve:", state.ReserveAmmo)
end
