...
function TweenLibrary:fadeGroup(canvasGroup, targetTransparency, duration, groupName)
	local info = TweenInfo.new(duration or 0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
	return self:tween(canvasGroup, {GroupTransparency = targetTransparency}, info, groupName)
end

function TweenLibrary:slideIn(instance, fromDirection, duration, groupName)
	local originalPosition = instance:GetAttribute("OriginalPosition") or instance.Position
	instance:SetAttribute("OriginalPosition", originalPosition)

	local offscreen = DIRECTIONS[fromDirection] or DIRECTIONS.bottom
...
function TweenLibrary:slideOut(instance, toDirection, duration, groupName)
	local offscreen = DIRECTIONS[toDirection] or DIRECTIONS.bottom
	local info = TweenInfo.new(duration or 0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
	return self:tween(instance, {Position = offscreen}, info, groupName)
end

function TweenLibrary:scaleIn(instance, duration, groupName)
	local originalScale = instance:GetAttribute("OriginalScale") or 1
	...
	return self:tween(uiScale, {Scale = originalScale}, info, groupName)
end
