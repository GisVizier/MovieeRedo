...
function Cache:_GetNew(Amount: number, Warn: boolean)
	...
	for Index = InitialLength + 1, InitialLength + Amount do
		...
		local ObjectRoot: BasePart = if IsTemplateModel
			then (Object :: Model).PrimaryPart :: BasePart
			else Object :: BasePart
...
function Cache:GetPart(PartCFrame: CFrame?): BasePart
	local Part = table.remove(self._FreeObjects)

	-- Keep trying until we get a valid (non-destroyed) part
	while Part and (Part.Parent == nil or Part:GetAttribute("__destroyed")) do
		Part = table.remove(self._FreeObjects)
	end

	-- If no valid parts, create new ones
	if not Part then
		Part = self:_GetNew(self._ExpandAmount, true)
	end
...
function Cache:ReturnPart(Part: BasePart)
	-- Don't return destroyed or already-cached parts
...
function Constructor.new(Template: BasePart | Model, CacheSize: number?, CachesContainer: Instance?)
	...
	assert(
		...
		`Invalid argument #1 to 'ObjectCache.new' (BasePart expected, got {TemplateType})`
	)
	...
	assert(
		CacheSize == nil or CacheSizeType == "number",
		`Invalid argument #2 to 'ObjectCache.new' (number expected, got {CacheSizeType})`
	)
	assert(
		CacheSize == nil or CacheSize >= 0,
		`Invalid argument #2 to 'ObjectCache.new' (positive number expected, got {CacheSize})`
	)

	local ContainerType = typeof(CachesContainer)
	assert(
		CachesContainer == nil or ContainerType == "Instance",
		`Invalid argument #3 to 'ObjectCache.new' (Instance expected, got {ContainerType})`
	)
