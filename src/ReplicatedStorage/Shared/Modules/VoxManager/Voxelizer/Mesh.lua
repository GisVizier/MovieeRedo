...
-- [ CONSTANTS ]
local MAX_MERGE_PASSES = 5 -- Reduced from 8; diminishing returns after ~4
...
function Mesh.tryMergeBlocks(blockA, blockB, tol)
	tol = tol or 0.001
	...
	if abs(aMinY - bMinY) < tol and abs(aMaxY - bMaxY) < tol
		and abs(aMinZ - bMinZ) < tol and abs(aMaxZ - bMaxZ) < tol then
	...
	if abs(aMinX - bMinX) < tol and abs(aMaxX - bMaxX) < tol
		and abs(aMinZ - bMinZ) < tol and abs(aMaxZ - bMaxZ) < tol then
	...
	if abs(aMinX - bMinX) < tol and abs(aMaxX - bMaxX) < tol
		and abs(aMinY - bMinY) < tol and abs(aMaxY - bMaxY) < tol then
