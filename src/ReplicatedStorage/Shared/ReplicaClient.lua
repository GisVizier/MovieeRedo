...
export type Replica = {
	...
	OnSet: (self: any, path: {}, listener: () -> ()) -> any,
	OnWrite: (self: any, function_name: string, listener: (...any) -> ()) -> any,
	OnChange: (self: any, listener: (action: string, path: { any }, param1: any, param2: any?) -> ()) -> any,
...
function Replica.OnNew(token: string, listener: (replica: Replica) -> ()): any
	if type(token) ~= "string" then
		error('[ReplicaClient]: "token" must be a string')
	end
...
function Replica:OnSet(path: {}, listener: () -> ()): any
...
function Replica:OnWrite(function_name: string, listener: (...any) -> ()): any
...
function Replica:OnChange(listener: (action: string, path: { any }, param1: any, param2: any?) -> ()): any
...
function Replica:Set(path: { string }, value: any)
	if ReplicationFlag ~= true then
		error('[ReplicaClient]: "Set()" can\'t be called outside of WriteLibs client-side')
	end
...
function Replica:SetValues(path: { string }, values: { [string]: any })
	if ReplicationFlag ~= true then
		error('[ReplicaClient]: "SetValues()" can\'t be called outside of WriteLibs client-side')
	end
...
function Replica:TableInsert(path: { string }, value: any, index: number?): number
	if ReplicationFlag ~= true then
		error('[ReplicaClient]: "TableInsert()" can\'t be called outside of WriteLibs client-side')
	end
...
function Replica:TableRemove(path: { string }, index: number): any
	if ReplicationFlag ~= true then
		error('[ReplicaClient]: "TableRemove()" can\'t be called outside of WriteLibs client-side')
	end
...
function Replica:Write(function_name: string, ...): ...any
	if ReplicationFlag ~= true then
		error('[ReplicaClient]: "Write()" can\'t be called outside of WriteLibs client-side')
	end
...
local function ReplicaFromBindBuffer(replica, announce_buffer)
	...
	if top_call == true then
		for _, repl in ipairs(announce_buffer) do
			local listeners = NewReplicaListeners[repl.Token]
			if listeners ~= nil then
				listeners:Fire(repl)
			end
		end
...
local function BreadthCreationSort(creation: {}, root_id: number?, iterator: (Id: number, SelfCreation: {}) -> ())
	local top_creation = {}
	local nested_creation = {}
	local error_creation = {}

	if type(creation[1]) == "table" then
	...
	if #error_creation ~= 0 then
		local msg = "[ReplicaClient]: GROUP REPLICATION ERROR - Missing parents for:\n"
		...
		if #error_creation > 50 then
			msg = msg .. "(hiding " .. (#error_creation - 50) .. " more)\n"
		end
...
local function GetInternalReplica(id)
	...
	if replica == nil then
		error("[ReplicaClient]: Received update for missing replica [Id:" .. id .. "]")
	end
	return replica
end
...
RemoteSet.OnClientEvent:Connect(function(id: number, path: {}, value: any)
	...
	if success ~= true then
		error("[ReplicaClient]: Error while updating replica:\n" .. replica:Identify() .. "\n" .. tostring(msg))
	end
end)
...
RemoteCreate.OnClientEvent:Connect(function(creation: {}, root_id: number?)
	local announce_buffer = {}

	BreadthCreationSort(creation, root_id, function(id: number, self_creation: {})
		local parent_id = self_creation[4]
...
for _, instance in pairs(CollectionService:GetTagged(CS_TAG)) do
	if instance:IsA("NumberValue") == true then
		OnBindInstanceAdded(instance)
	end
end
